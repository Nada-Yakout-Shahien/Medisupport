...
const Profile = () => {
    ...
    async function fetchUserProfile() {
      try {
        ...
        const response = await axios.get(
          "http://127.0.0.1:8000/api/auth/user/user-profile",
          {
            headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (imageSrc) {
        formData.append("avatar", user.imageSrc);
      }

      await axios.post(
        "http://127.0.0.1:8000/api/auth/user/update-profile",
        formData,
        axiosConfig
      );
  ...
  const deleteData = async () => {
    try {
      ...
      await axios.delete(
        "http://127.0.0.1:8000/api/auth/user/delete-account",
        axiosConfig
      );
    ...
    <Layout>
      ...
      <div className="profile-sec">
        ...
        {imageSrc ? (
          ...
          user &&
          user.avatar && (
            <img src={`${BASE_URL}${user.avatar}`} alt="User Avatar" />
          )
        )}
        <div className="edit">
        ...
        {/* User Name */}
        {user && <h4>{`${user.first_name || ""} ${user.last_name || ""}`}</h4>}
      </div>
      ...
      <div className="info-sec">
        ...
        {/* Buttons */}
        <div className="save">
          <button type="submit" onClick={() => setShowPopupS(true)}>
            Save
          </button>
          {showPopupS && (
          ...
          {showPopupD && (
            <div className="popup-d">
              <div className="message">
                ...
                <p>
                  Aliquam vivamus elementum dictum
                  <br />
                  suspendisse enim metus.
                </p>
